...
struct strict_integers
{
    using serialize = zpp::bits::strict_members<2>;
    int x;
    int y;
};

struct strict_integers_auto
{
    using serialize = zpp::bits::strict_members<>;
    int x;
    int y;
};
...
static_assert(!zpp::bits::concepts::byte_serializable<strict_integers>);
static_assert(!zpp::bits::concepts::byte_serializable<strict_integers_auto>);
...
public:
    ...
    std::strong_ordering operator<=>(
        const inaccessible & other) const = default;
...
struct strict_requires_padding
{
    using serialize = zpp::bits::strict_members<2>;
    std::byte b{};
    std::int32_t i32{};
...
TEST(byte_serializable, strict_requires_padding)
{
    static_assert(
        !zpp::bits::concepts::byte_serializable<strict_requires_padding>);
    static_assert(sizeof(requires_padding) ==
                  sizeof(strict_requires_padding));
    static_assert(sizeof(strict_requires_padding) > 5);

    auto [data, in, out] = zpp::bits::data_in_out();
    out(strict_requires_padding{std::byte{0x25}, 0x1337}).or_throw();
...
struct strict_requires_padding_auto
{
    using serialize = zpp::bits::strict_members<>;
    std::byte b{};
    std::int32_t i32{};
...
static_assert(sizeof(requires_padding) == sizeof(strict_requires_padding_auto));

TEST(byte_serializable, strict_requires_padding_auto)
{
    static_assert(!zpp::bits::concepts::byte_serializable<
                  strict_requires_padding_auto>);
    static_assert(sizeof(requires_padding) ==
                  sizeof(strict_requires_padding_auto));
    static_assert(sizeof(strict_requires_padding_auto) > 5);

    auto [data, in, out] = zpp::bits::data_in_out();
    out(strict_requires_padding_auto{std::byte{0x25}, 0x1337}).or_throw();
    ...
    strict_requires_padding_auto s;
    in(s).or_throw();
    EXPECT_EQ(in.position(), std::size_t{5});
    EXPECT_EQ(s.b, std::byte{0x25});
    EXPECT_EQ(s.i32, 0x1337);
